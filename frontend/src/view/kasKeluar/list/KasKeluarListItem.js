...
import selectors from 'modules/kasKeluar/kasKeluarSelectors';

class KasKeluarListItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    ...
    return <div key={record.id}>{record['catatan']}</div>;
  };
...
export default connect(select)(KasKeluarListItem);
