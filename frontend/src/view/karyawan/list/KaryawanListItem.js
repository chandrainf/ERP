...
class KaryawanListItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    ...
    return <div key={record.id}>{record['karyawan']}</div>;
  };
...
export default connect(select)(KaryawanListItem);
